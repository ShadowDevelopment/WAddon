...
import me.wiedzmin137.waddon.util.DragonUtil;
import me.wiedzmin137.waddon.util.FakeDragon;
...
public class BarAPI implements Listener {
	...
	private static WAddon plugin;

	public BarAPI(WAddon plugin) {
	...
	private void handleTeleport(final Player player, final Location loc) {
		...
		Bukkit.getScheduler().runTaskLater(plugin, new Runnable() {
			...
			public void run() {
				...
				DragonUtil.sendPacket(player, getDragon(player, "").getDestroyPacket());
	...
	private static void sendDragon(FakeDragon dragon, Player player) {
		DragonUtil.sendPacket(player, dragon.getMetaPacket(dragon.getWatcher()));
		DragonUtil.sendPacket(player, dragon.getTeleportPacket(player.getLocation().add(0, -300, 0)));
	}
	...
	private static FakeDragon addDragon(Player player, String message) {
		FakeDragon dragon = DragonUtil.newDragon(message, player.getLocation().add(0, -300, 0));

		DragonUtil.sendPacket(player, dragon.getSpawnPacket());
	...
	private static FakeDragon addDragon(Player player, Location loc, String message) {
		FakeDragon dragon = DragonUtil.newDragon(message, loc.add(0, -300, 0));
