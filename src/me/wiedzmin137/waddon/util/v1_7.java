...
public class v1_7 extends FakeDragon {
	...
	public Object getSpawnPacket() {
		Class<?> Entity = DragonUtil.getCraftClass("Entity");
		Class<?> EntityLiving = DragonUtil.getCraftClass("EntityLiving");
		Class<?> EntityEnderDragon = DragonUtil.getCraftClass("EntityEnderDragon");
		Object packet = null;
		try {
			dragon = EntityEnderDragon.getConstructor(DragonUtil.getCraftClass("World")).newInstance(getWorld());

			Method setLocation = DragonUtil.getMethod(EntityEnderDragon, "setLocation", new Class<?>[] { double.class, double.class, double.class, float.class, float.class });
			setLocation.invoke(dragon, getX(), getY(), getZ(), getPitch(), getYaw());

			Method setInvisible = DragonUtil.getMethod(EntityEnderDragon, "setInvisible", new Class<?>[] { boolean.class });
			setInvisible.invoke(dragon, isVisible());

			Method setCustomName = DragonUtil.getMethod(EntityEnderDragon, "setCustomName", new Class<?>[] { String.class });
			setCustomName.invoke(dragon, name);

			Method setHealth = DragonUtil.getMethod(EntityEnderDragon, "setHealth", new Class<?>[] { float.class });
			setHealth.invoke(dragon, health);

			Field motX = DragonUtil.getField(Entity, "motX");
			motX.set(dragon, getXvel());

			Field motY = DragonUtil.getField(Entity, "motY");
			motY.set(dragon, getYvel());

			Field motZ = DragonUtil.getField(Entity, "motZ");
			motZ.set(dragon, getZvel());

			Method getId = DragonUtil.getMethod(EntityEnderDragon, "getId", new Class<?>[] {});
			this.id = (Integer) getId.invoke(dragon);

			Class<?> PacketPlayOutSpawnEntityLiving = DragonUtil.getCraftClass("PacketPlayOutSpawnEntityLiving");
	...
	public Object getDestroyPacket() {
		Class<?> PacketPlayOutEntityDestroy = DragonUtil.getCraftClass("PacketPlayOutEntityDestroy");
	...
	public Object getMetaPacket(Object watcher) {
		Class<?> DataWatcher = DragonUtil.getCraftClass("DataWatcher");

		Class<?> PacketPlayOutEntityMetadata = DragonUtil.getCraftClass("PacketPlayOutEntityMetadata");
	...
	public Object getTeleportPacket(Location loc) {
		Class<?> PacketPlayOutEntityTeleport = DragonUtil.getCraftClass("PacketPlayOutEntityTeleport");
	...
	public Object getWatcher() {
		...
		try {
			...
			Method a = DragonUtil.getMethod(DataWatcher, "a", new Class<?>[] { int.class, Object.class });
